# Tutorial: building a latent diffusion model from scratch (PyTorch)

This tutorial walks you through `plan.md` as a hands-on exercise. The rule: **you write every line yourself**. I only step in to explain concepts, review code, or unblock you when you're truly stuck.
...
- [ ] Explain why we work in latent space and not pixel space.
...
**Goal:** a working PyTorch + CUDA environment on Colab.
...
- [x] Run `torch.cuda.is_available()` and `torch.cuda.get_device_name(0)` to confirm a GPU is visible.
- [ ] Mount Google Drive and create a folder like `/content/drive/MyDrive/ddl-diffusion/` for checkpoints and precomputed data.
...
You only run this once. After it's done, restart the runtime to free VRAM and don't touch the VAE or CLIP again until inference.
...
4. **Encode one image** through the VAE first. Use `vae.encode(x).latent_dist.sample()`. Check the output shape, which should be `(1, 4, 64, 64)`. Multiply by `0.18215`.
...
7. **Load the CLIP tokenizer and text encoder** from `openai/clip-vit-large-patch14`.
...
- [ ] Sanity-check: take one latent, divide by `0.18215`, decode through `vae.decode(latent).sample`, denormalize, display. It should look like the original Pokemon.
- [ ] After saving, restart the runtime. From here on, you don't need diffusers or transformers loaded during training.
...
### Tasks
- [ ] Write `NoiseScheduler`. A small `nn.Module` is convenient, since registering tensors as buffers moves them with `.to(device)`.
...
- [ ] **Test it:** load one latent, sample noise, run `q_sample` at `t=0`, `t=500`, `t=999`. Decode each through the VAE (in a separate cell, reload the VAE briefly). You should see a clean Pokemon, a very noisy Pokemon, and pure noise.
...
- [ ] Use `F.scaled_dot_product_attention(q, k, v)`, which is fast and fused.
...
- [ ] Same multi-head pattern, but K/V have sequence length 77 and Q has length H*W.
...
**Checkpoint:** the forward pass works on real precomputed latents and embeddings without crashing. The output is garbage, which is expected. Move on.
...
- [ ] Move `latents`, `embeddings`, `uncond_embedding` to GPU once if they fit. 833 × 4 × 64 × 64 × 4 bytes ≈ 50MB, so they will.
- [ ] Instantiate `model`, `scheduler` (NoiseScheduler), `optimizer = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=0.01)`.
- [ ] Build the LR schedule: warmup 1000 steps, then cosine to 0. `transformers.get_cosine_schedule_with_warmup` is a one-liner.
- [ ] Build the EMA model: a deep-copied `nn.Module` with `requires_grad_(False)` on all params.
...
- [ ] Wrap with `torch.compile(model)` and confirm it still trains. The first step is slow while it compiles.
- [ ] Add running-loss logging every 50 steps.
- [ ] Add a checkpoint save every 25 epochs: `{model_state, ema_state, optimizer_state, lr_scheduler_state, step, epoch}` to Drive.
- [ ] Add resume-from-checkpoint logic.
- [ ] Train for 50 epochs as a first real run.

**Checkpoint:** the loss curve goes down and plateaus around some non-zero value. Save a checkpoint to Drive.
...
- [ ] Take the final `x_0`, divide by `0.18215`, reload the VAE briefly, decode, denormalize to [0,1], display with matplotlib.

**Checkpoint:** the image is no longer pure noise. Probably blurry, possibly cursed-looking. That's fine, push training longer if needed.
...
- [ ] Implement the DDIM update with `eta=0` (deterministic):
...
- [ ] Wrap forward and loss in `with torch.autocast(device_type="cuda", dtype=torch.bfloat16):`.
...
- [ ] Cast the model: `model.to(dtype=torch.bfloat16)`. Cast inputs to bf16 before the forward pass.
...
| Generated images = noise | Train longer; check CFG actually swaps to uncond; verify the EMA model is the one used at inference |
| Generated images = solid color | Sign error in the DDPM update; or the model is in `train()` mode at inference (BN/dropout active) |
| Forward pass works, training breaks | A `.detach()` somewhere is killing grads; or a tensor was moved to the wrong device |
...
Writing it yourself is uncomfortable and slow, and it is the only part that actually teaches you anything.